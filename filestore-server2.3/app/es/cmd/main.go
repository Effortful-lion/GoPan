...
import (
	"filestore-server/app/es/service"
...
func registerGrpcService() {
	...
	cfg := gRPCRegisterConfig{
		Addr: config.Config.ServiceConfig.ESServiceAddress,
		RegisterFunc: func(g *grpc.Server) {
			esPb.RegisterESServiceServer(g, service.NewESService())
		},
	}
...
func registerEtcdService() {
	...
	info := discovery.Server{
		Name: config.Config.Domain.ESSServiceDomain,
		Addr: config.Config.ServiceConfig.ESServiceAddress,
	}
	logrus.Println(info)
