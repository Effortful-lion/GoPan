// 数据库代理服务 8085
// 启动后，注册服务，其他服务通过rpc进行远程调用
package main

import (
	"filestore-server/app/dbproxy/mysql"
	"filestore-server/app/dbproxy/service"
...
func registerGrpcService() {
	...
	cfg := gRPCRegisterConfig{
		Addr: config.Config.ServiceConfig.DBProxyServiceAddress,
		RegisterFunc: func(g *grpc.Server) {
			dbproxyPb.RegisterDBProxyServiceServer(g, service.NewDBProxyService())
			log.Println("DBProxyService registered successfully")
		},
	}
...
func registerEtcdService() {
	...
	info := discovery.Server{
		Name: config.Config.Domain.DBProxyServiceDomain,
		Addr: config.Config.ServiceConfig.DBProxyServiceAddress,
	}
	logrus.Println(info)
