...
func (s *UserService) SignInHandlerPost(ctx context.Context, req *userPb.SigninRequest) (*userPb.SigninResponse, error) {
	...
	if rpc.DBProxyClient == nil {
		log.Println("DBProxyClient is nil")
	}
	...
	_, err = rpc.UpdateToken(ctx, &dbproxyPb.UpdateTokenRequest{
		Username: username,
		Token:    token,
	})
	if err != nil {
...
func (s *UserService) UserInfoHandler(ctx context.Context, req *userPb.UserInfoRequest) (*userPb.UserInfoResponse, error) {
	...
	// 3. 查询用户信息
	// 4. 组装并返回响应数据
	res, err := rpc.GetUserInfo(ctx, &dbproxyPb.GetUserInfoRequest{
		Username: username,
	})
	if err != nil {
		return nil, err
