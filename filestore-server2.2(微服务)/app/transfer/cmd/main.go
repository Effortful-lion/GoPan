...
import (
	...
	"filestore-server/app/transfer/rpc"
...
func ProcessTransfer(msg []byte) bool {
	...
	// 判断目标存储类型
	switch pubData.DestStoreType {
	case common.StoreCeph:
		// 3. 通过文件句柄将文件内容读出来并且上传到ceph
		fmt.Println("开始上传文件到ceph:", pubData.DestLocation)
		data, _ := io.ReadAll(fd)
		bucket := ceph.GetCephBucket(config.Config.CephConfig.CephBucket)
		err := bucket.Put(pubData.DestLocation, data, "octet-stream", s3.PublicRead)
		if err != nil {
			log.Println(err.Error())
			return false
		}
		// 删除临时文件
		_ = os.Remove(pubData.CurLocation)
	case common.StoreOSS:
		// 3. 通过文件句柄将文件内容读出来并且上传到oss
		fmt.Println("开始上传文件到oss:", pubData.DestLocation)
		...
		// 删除临时文件
		_ = os.Remove(pubData.CurLocation)
	case common.StoreLocal:
		// 退出
		return true
	case common.StoreMix:
		// 3.1 先上传到ceph
		fmt.Println("开始上传文件到ceph:", pubData.DestLocation)
		data, _ := io.ReadAll(fd)
		err := ceph.PutObject(config.Config.CephConfig.CephBucket, pubData.DestLocation, data)
		if err != nil {
			log.Println(err.Error())
			return false
		}
	...
	// 4. 更新文件的存储路径到文件表
	_, err = rpc.UpdateFileLocation(context.Background(), &dbproxyPb.UpdateFileLocationRequest{
		Filehash: pubData.FileHash,
