...
import (
	...
	"filestore-server/app/upload/service"
...
func registerGrpcService() {
	...
	cfg := gRPCRegisterConfig{
		Addr: config.Config.ServiceConfig.UploadServiceAddress,
		RegisterFunc: func(g *grpc.Server) {
			uploadPb.RegisterUploadServiceServer(g, service.NewUploadService())
		},
	}
...
func registerEtcdService() {
	...
	info := discovery.Server{
		Name: config.Config.Domain.UploadServiceDomain,
		Addr: config.Config.ServiceConfig.UploadServiceAddress,
	}
	logrus.Println(info)
